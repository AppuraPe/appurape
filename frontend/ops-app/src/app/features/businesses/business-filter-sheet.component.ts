...
@Component({
  ...
  template: `
    @if (isOpen()) {
      ...
      <aside class="fixed inset-x-0 bottom-0 z-[91] max-h-[78dvh] overflow-y-auto rounded-t-[24px] border border-[#ead8d2] bg-white px-4 pb-[calc(env(safe-area-inset-bottom,0px)+0.75rem)] pt-3 shadow-[0_-14px_36px_rgba(6,25,43,0.16)] lg:hidden">
        ...
        <div class="flex items-center justify-between gap-3">
          <div>
            ...
            <p class="text-sm text-text-muted">Encuentra negocios según categoría, zona y disponibilidad.</p>
          </div>
          <button type="button" class="text-sm font-semibold text-primary-700" (click)="clearFilters.emit()">Limpiar</button>
        </div>

        <form class="mt-4 grid gap-3" [formGroup]="form()">
          <label class="grid gap-2">
            <span class="text-sm font-semibold text-loreto-carbon">Categoría</span>
            <select class="min-h-12 w-full rounded-[14px]" formControlName="businessTypeId">
          ...
          <label class="grid gap-2">
            ...
            <select class="min-h-12 w-full rounded-[14px]" formControlName="zoneId" [disabled]="isLoadingZones() && !zones().length">
          ...
          <label class="grid gap-2">
            ...
            <select class="min-h-12 w-full rounded-[14px]" formControlName="sort">
        ...
        <div class="mt-4 grid grid-cols-2 gap-2">
          <button type="button" class="button secondary min-h-11 w-full" (click)="close.emit()">Cerrar</button>
          <button type="button" class="button min-h-11 w-full" (click)="close.emit()">Aplicar filtros</button>
        </div>
      </aside>
    }
  `,
