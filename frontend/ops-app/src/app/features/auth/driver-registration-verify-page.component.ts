...
export class DriverRegistrationVerifyPageComponent {
  ...
  verify(): void {
    ...
    this.authApi
      .verifyDriverRegistrationCode({ email: raw.email.trim(), code: raw.code.trim() })
  ...
  resend(): void {
    ...
    this.authApi
      .resendDriverRegistrationCode({ email })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (response) => {
