    ...
    <section class="auth-page">
      <div class="page-card auth-card">
        <app-page-header
          ...
          subtitle="Completa el registro del restaurante y entra al panel AppuraPe."
        />

        <app-notice
          tone="warning"
          title="Paso 3 de 3"
          message="Al crear la cuenta entraras al panel, pero el restaurante quedara pendiente de aprobacion antes de aparecer al publico o recibir pedidos."
        />
        ...
        <form class="form-grid" [formGroup]="form" (ngSubmit)="complete()">
          ...
          <div class="page-actions">
            ...
            <a class="button ghost" routerLink="/register/restaurant/verify">Volver</a>
          </div>
        </form>
      </div>
    </section>
    ...
    this.authService
      ...
      .subscribe({
        next: () => {
          ...
          setTimeout(() => void this.router.navigateByUrl('/restaurant/dashboard'), 1500);
        },
        error: (error) => {
          this.errorMessage.set(getErrorMessage(error, 'No se pudo completar el registro del restaurante.'));
          this.isSubmitting.set(false);
        },
      });
  }
